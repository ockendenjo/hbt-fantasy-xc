...
func (lb *LambdaBuilder) WithQueuePublish(queueWithDLQ QueueWithDLQ, queueUrlEnvKey string) *LambdaBuilder {
	queueWithDLQ.Queue.GrantSendMessages(lb.role)
	lb.env[queueUrlEnvKey] = queueWithDLQ.Queue.QueueUrl()
	return lb
}
